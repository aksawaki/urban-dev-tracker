...
_BAD_TITLE_KEYWORDS: frozenset[str] = frozenset([
    ...
    # 鉄道ダイヤ改正・車両リニューアル（運行計画であり開発計画でない）
    "ダイヤ改正", "時刻改正", "ダイヤ変更", "車両リニューアル", "新車両",
    # 屋外広告・景観条例（行政制度ページ）
    "屋外広告業登録", "屋外広告物", "景観条例",
    ...
    # 動物・自然（都市開発でない）
    "が死亡", "コアラ", "動物公園", "動物園",
    # 避難施設・国民保護（行政通知、都市開発でない）
    "緊急一時避難施設", "国民保護法",
    # 単体マンション・共住（kensetsunews の単発着工記事、大規模再開発でない）
    "に共住",
    # 面積指定の解体工事（kensetsunews の単体解体契約、都市開発でない）
    "㎡解体",
    # 高等専門学校施設（教育施設、都市開発でない）
    "高専",
    # スポーツ選手育成・誘致（都市開発でない）
    "FCバルセロナ", "選手育成機関", "Ｊ３規格",
    # 港湾浚渫・治水インフラ
    ...
    "ビルを取得",
    # 企業の入社式・社長あいさつ（企業PR、都市開発でない）
    "入社式", "社長あいさつ",
    # 民活調査（公共施設の民間活用調査・プロポ、都市開発でない）
    "民活調査",
    # 図書館・体育館複合施設PFI（都市開発でない公共施設）
    "図書館・体育館",
    # 図書館（公共施設、都市開発でない）
    "図書館",
    # ミュージアム・美術館（文化施設単体、都市開発でない）
    "ミュージアム", "美術館",
    # 電子契約・ITシステム（不動産業務DX、都市開発でない）
    "電子契約",
    ...
    "文化複合施設",
    # 解体工事（単体建替・既存建物解体、都市開発でない）
    "ビル解体", "建物解体",
    # 砂防・治山工事（地方インフラ、都市開発でない）
    "砂防",
    # 宇宙・JAXA研究（都市開発でない先端技術）
    "JAXA", "宇宙での",
    # 海洋・港湾工事（都市開発でない）
    "ケーブル敷設船", "港湾工事",
    # 海外インフラ・発電（都市開発でない）
    ...
    # 教育施設（義務教育校、都市開発でない）
    "義務教育校",
    # 発注見通し（行政の発注予定一覧、都市開発でない）工事・業務どちらも除外
    "発注見通し",
    # 市場再整備（卸売市場、都市開発でない）
    "市場再整備",
    # コーディネーター募集・空き家活用（行政募集、都市開発でない）
    "コーディネーター募集",
    # 海外ビジネス交流・セミナー（不動産PRイベント、都市開発でない）
    "INDIA CONNECT", "インドへの進出",
    # 治安・犯罪対策キャンペーン（行政通知、都市開発でない）
    "痴漢撲滅", "春の安全",
    # 寺院・共住建替（kensetsunews の寺院兼共住着工記事、都市開発でない）
    "兼共住",
    # PFI実現可能性調査（調達通知、都市開発でない）
    "PFI調査",
    # 文化ホール・会館（公共文化施設、都市開発でない）
    "文化会館",
    # 劣化調査（既存建物の点検・調査業務、都市開発でない）
    "劣化調査",
    # 発注者支援業務（行政のコンサル調達、都市開発でない）
    "発注者支援",
    # 県庁再整備（行政庁舎改修、都市開発でない）
    "県庁再整備",
    # 河川国道（道路・河川インフラ管理、都市開発でない）
    "河川国道",
    # 統合中学校（学校統廃合施設建設、都市開発でない）
    "統合中学校",
    # 公園再整備（緑地整備、都市開発でない）
    "公園再整備",
    # 動物愛護センター（公共施設、都市開発でない）
    "動物愛護",
    # CM追加契約パターン（河川・事務所等のCM業務、都市開発でない）
    "ほかCM",
    # 廃食用油・環境資源回収（都市開発でない）
    "廃食用油",
    # 物流施設（倉庫・配送センター建設、都市開発でない）
    "物流施設",
    # 再開発準備組合支援（コンサル調達、都市開発でない）
    "準備組合支援",
    # イノベーション創出・リーディングプロジェクト（行政募集、都市開発でない）
    "イノベーション創出",
    # 統合小学校（学校統廃合施設、都市開発でない）
    "統合小",
    # 学校本館・校舎解体（都市開発でない）
    "本館解体",
    # 公園複合施設DBO（公園整備、都市開発でない）
    "公園複合施設",
    # 工場・生産棟（産業施設、都市開発でない）
    "新生産棟", "生産棟着工",
    # 最終処分場（廃棄物処理施設、都市開発でない）
    "最終処分場",
    # 火葬場（葬祭施設、都市開発でない）
    "火葬場",
    # 風力発電（再生可能エネルギー施設、都市開発でない）
    "風力発電",
    # ごみ処理・廃棄物広域化（都市開発でない）
    "ごみ処理",
    # ギャンブル依存症対策（公共施設、都市開発でない）
    "ギャンブル依存症",
    # 病院建設（医療施設、都市開発でない）
    "新病院",
    # 国際スポーツ大会支援（都市開発でない）
    "国際スポーツ大会",
    # 産業拠点形成調査（工業用地の市場調査、都市開発でない）
    "産業拠点形成",
    # ポンプ場（上下水道インフラ、都市開発でない）
    "ポンプ場",
    # 庁舎建替（行政庁舎整備、都市開発でない）
    "庁舎建替",
    # 住宅貸主支援事業（行政補助制度、都市開発でない）
    "貸主応援",
    # 川崎・創造のもり（研究施設再編、都市開発でない）
    "創造のもり",
...
_BAD_TITLE_PATTERNS: list[str] = [
    ...
    # 「○月○日まで参加受付/タイトル/地域」型（kensetsunews の調達告知）
    r"^\d+月\d+日まで参加受付/",
    # 団地全域建替基本構想（公営住宅建替の行政計画、都市開発でない）
    r"団地.{0,5}(全域建替|建替基本構想)",
    # まちづくり支援施設の基本計画（行政委託調達、都市開発でない）
    r"まちづくり支援施設.{0,10}基本計画",
    # 単体マンション着工・完成（大規模再開発でない単発マンション記事）
    r"[都道府県市区町村]のマンション.{0,5}(着工|完成)",
    # 「○○市に○万㎡のマンション、」型（面積つき単体マンション記事）
    r"[都道府県市区町村]に[\d.万㎡]+のマンション[、,]",
    # 落札・契約決定（kensetsunews の「(約)○億で施工者/工事名/」形式）
    r"^約?\d+億.{0,3}で[^\s／/]{2,15}/",
    r"^約?\d+億.{0,3}で[^\s／/]{2,15}／",
    # 単体建物の着工告知（kensetsunews の「施工者で/に(○月)着工/場所+建物種別」形式）
    r"[にで].{0,5}着工[/／]",
    # 単体建物の着工発表（「○○が着工、施工は○○」形式、再開発でない個別建物）
    r"が着工[、,]施工は",
    # 「○○に決まる/工事名/発注者」（落札決定告知、都市開発でない公共工事調達）
    r"に決まる[/／]",
    # アリーナのPFI・入札・基本計画（スポーツ施設調達、都市開発でない）
    r"アリーナ.{0,20}(PFI|基本計画|入札|建設工事|不調)",
    # 工事金額リスト記事（kensetsunews の入札結果一覧）
    r"工事\d+億以上が",
    # 基本計画発注支援（コンサル調達、都市開発でない）
    r"基本計画発注支援",
    # 基本構想案を公表（文化・公共施設計画の初期段階）
    r"基本構想案を公表",
    # キャンペーン・撲滅・啓発（行政PR、都市開発でない）
    r"(キャンペーン|撲滅|啓発).{0,10}(実施|を実施|開始)",
    # 海外ビジネス交流イベント（不動産PR、都市開発でない）
    r"(CONNECT|交流会|経済交流).{0,20}(開催|テーマ|インド|海外)",
    # CMコンサルタント選定記事（kensetsunews の公共施設CM選定、都市開発でない）
    r"CMを特定[/／]",
    # 単体ホテル完成・竣工・開業（都市開発でない単発ホテル記事）
    r"にホテル.{0,10}(完成|竣工|開業|目指)",
    # 運輸・物流会社の支店・施設着工（都市開発でない）
    r"(運輸|運送).{0,10}(新施設|支店|センター).{0,10}着工",
    # 飲食店・小規模店舗の解体（都市開発でない）
    r"[店屋舗]を解体[/／]",
    # 「○月○日-○日に受付/工事名/発注者」型の再公告告知（調達通知、都市開発でない）
    r"\d+月\d+[-〜]\d+日に受付[/／]",
    # 落札決定告知「〜に決定/工事名」（公共工事調達、都市開発でない）
    r"に決定[/／]",
]
